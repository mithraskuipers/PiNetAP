...
class PiNetAP:
    ...
    def list_interfaces(self, detailed: bool = False):
        ...
        if wifi_count < 1:
            ...
            print(f"   sudo python pinetap.py install --ssid MyHotspot --password Pass123 \\")
...
def main():
    parser = argparse.ArgumentParser(
        description="PiNetAP - Dual WiFi Access Point Manager for Raspberry Pi (MAC-Stable)",
...
Examples:
  ...
  # Test interface consistency after reboot
  %(prog)s test --type prerequisites

  # Full test suite
  sudo %(prog)s test --type all --connection MyHotspot-AP --ssid MyHotspot

Note: Connections are bound to MAC addresses, so interface names (wlan0/wlan1) 
      may change after reboot, but your AP will continue to work correctly.
"""
    )
    ...
    uninstall_parser.add_argument("--connection", required=True, 
